...
public class asset {
    ...
    private List<Integer> textureID;                                          //textureID in memory
    ...
    public asset(Context context, int textureID){                       //Initialize with context and textureID from res (e.g. R.drawable.npc_front)
        ...
        this.textureID = new ArrayList<Integer>();
        this.textureID.add(loadTexture(this.context, textureID));      //load texture/bitmap
    }
    public void addTexture(int textureID){
        this.textureID.add(loadTexture(this.context, textureID));
    }
    /*-----------------Draw function-------------*/
    //mvp = Model-View-Projection matrix
    //coordinates in openGL. coords should already be converted to floatbuffer
    public void draw(float[] mvpMatrix, FloatBuffer coords, int texIndex) {
        vertexBuffer = coords;                                                          //position of quadrangle relative to GL coordinates
        GLES20.glBindTexture(GLES20.GL_TEXTURE_2D, this.textureID.get((texIndex)%textureID.size()));                     //Bind a texture ID so GL knows what texture to use
